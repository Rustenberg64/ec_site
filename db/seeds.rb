...
product = Product.new(
  ...
  stock: rand(50),
  created_at: Time.now.ago(10.hours),
  updated_at: Time.now.ago(10.hours)
)
product.image.attach(io: File.open("./db/fixtures/bunbougu_enpitsu.png"), filename: 'bunbougu_enpitsu.png')
product.save

datetime_from = Time.now.ago(20.years)
datetime_to   = Time.now.ago(1.day)

# 1
date = rand(datetime_from..datetime_to)
product = Product.new(
  name: "水筆",
  manufacturer: Faker::Company.name,
  price: 1080,
